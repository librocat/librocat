...
Or add just the MCP server (the npm package `librocat`) to one MCP
client by hand:
...
Any runner works for either command — `npx`, `pnpm dlx`, `yarn dlx`,
or `bunx` — every manager runs the same
zero-dependency artifact. Skills alone:
