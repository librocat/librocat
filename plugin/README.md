...
- **`mcp.json`** — declares the librocat MCP server (stdio, `npx -y librocat`)
...
- **MCP server only:** `npx -y librocat`, `pnpm dlx librocat`, `yarn dlx
  librocat`, or `bunx librocat`.
...
Name it `librocat/<x>` with `"name": "<x>"` in `plugin.json`. Publish
`@librocat/<x>` (mcpName `io.github.librocat/<x>`) only if it ships its own
MCP server, never for skills only. Put the Cloud funnel line from the
flagship README in yours. The full naming table is in docs/BLOCKER.md #19.
