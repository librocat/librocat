...
// The plugin carries its own server: build it and commit the bundle form.
const build = spawnSync("pnpm", ["--filter", "@librocat/server", "run", "build"], {
  cwd: root,
  stdio: "inherit",
});
...
fs.copyFileSync(
  path.join(root, "packages", "mcp", "dist", "main.js"),
